...
export default function ProjectItems({ onEdit, onDelete }: ProjectItemsProps) {
    ...
    <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {loading ? (
              <tr>
                <td colSpan={3} className="px-6 py-4 text-center text-gray-500">Loading items...</td>
              </tr>
              ...
              filteredItems.map((item) => (
                <tr key={item.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right">
                    <div className="text-sm font-medium text-gray-900">{formatCurrency(Number(item.cost))}</div>
                  </td>
      ...
      <div className="mt-4 flex justify-between items-center">
        <p className="text-sm text-gray-600">
          {loading ? "Loading..." : `Showing ${filteredItems.length} of ${items.length} items`}
        </p>
